import { addZero, pick, random } from '../utils/helper'
...
const birthday = (separator = '') => {
  ...
  return `${date.getFullYear()}${separator}${addZero(date.getMonth() + 1, 2)}${separator}${addZero(date.getDate(), 2)}`
}
...
class IDCard {
  ...
  // 第一、二位表示省级行政区。
  province = [11, 12, 13, 14, 15, 21, 22, 23, 31, 32, 33, 34, 35, 36, 37, 41, 42, 43, 44, 45, 46, 50, 51, 52, 53, 54, 61, 62, 63, 64, 65]
  // 第三、四位表示地级行政区；第五、六位表示县级行政区。
  city = ['0101', '0201']

  // 根据前17位计算第18位效验码
  getValidationCode = (total: any) => {
  ...
  init = () => {
    let iSum = 0
    const sId = pick(this.province) + pick(this.city) + birthday() + String(random(100, 999))
    for (let i = 17; i >= 1; i--) {
      iSum += (Math.pow(2, i) % 11) * parseInt(sId.charAt(17 - i), 11)
    }
    return sId + this.getValidationCode(iSum)
  }
}

const idcard = new IDCard().init
